...
public static class ShaderHelper
    {
        ...
        public static void AttachShaders(GLHandle programHandle, params Shader[] shaders)
        {
            ...
            {
                Assert.That(!shaders[i].Disposed);
        ...
        public static void BuildShaderInfo(ShaderProgram shaderProgram)
        {
            ...
            shaderProgram.UniformLocations = uniforms.Select(kvp => kvp.Key)
                                                     .ToDictionary(key => key, key => uniforms[key].Location);
            shaderProgram.UniformsByLocation = uniforms.Select(kvp => kvp.Value)
                                                       .ToDictionary(value => value.Location, value => value);
        }
    }
}